...
def minCostDPWithSegmentTree(cost, k):
    n = len(cost)
    segment_tree = SegmentTree(n)
    segment_tree.update(0, cost[0])

    # Store the minimum costs and paths
    min_costs = [float('inf')] * n
    min_costs[0] = cost[0]
    paths = [[0]]  # Initialize paths for the first platform
    ...
    for i in range(1, n):
        ...
        for j in range(start, i):
            potential_cost = min_costs[j] + cost[i]
        ...
        segment_tree.update(i, min_cost)
...
# Calculate the minimum cost and paths
min_costs, paths = minCostDPWithSegmentTree(cost, k)

# Results
print(min_costs[-1], paths[-1])
